...
class PoolPositionsDataSchema(Schema):
    ...
    positionType = fields.String(required=True, validate=validate_postion_type)
...
class PoolPositionsSchema(Schema):
    data = fields.Nested(PoolPositionsDataSchema, many=True)
    #provenance = fields.Nested(serverResponce)

class PoolPositionApyLeadersSchema(Schema):
    data = fields.Nested(PoolPositionsDataSchema, many=True)
    #provenance = fields.Nested(serverResponce)